...
class TypedCheckout(
  ...
  def start: Behavior[TypedCheckout.Command] =
    Behaviors.receive[TypedCheckout.Command]((context, command) =>
      command match {
        case StartCheckout =>
  ...
  def processingPayment(timer: Cancellable): Behavior[TypedCheckout.Command] =
    Behaviors.receive[TypedCheckout.Command]((_, command) =>
      command match {
  ...
  def cancelled: Behavior[TypedCheckout.Command] = Behaviors.receive[TypedCheckout.Command]((_, _) => Behaviors.stopped)

  def closed: Behavior[TypedCheckout.Command] = Behaviors.receive[TypedCheckout.Command]((_, _) => Behaviors.stopped)
